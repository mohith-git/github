...
def contact_map():
     ...
     cm = c_m(f)
     cm = pd.DataFrame({'Amino acid':cm[0],'Aminoacid':cm[1],'dist':cm[2]})
     fig = px.scatter(cm,x='Amino acid',y='Aminoacid',color ='dist',opacity=0.3,color_continuous_scale=px.colors.sequential.Bluyl,title='Contact Map     {}'.format(f),template='plotly_dark',width=750,height=550)
     fig.update_layout(
